...
short Pause=0;
short Stop=0;
__attribute__((section(".version_string"))) char version_string[20];
...
int main(void)
{
  ...
  // SSD_init();
   UART_OutChar('>');
   strcpy(version_string,"Version 1.1");
...
void UART_cmpInp()
{
  ...
  UART_InString(string,MAX_INP_BUFF_SIZE);
   ...
   if(inp_done){
    ...
    else if((!(strcmp(cmd_string,"peek")))&&(!Stop)&&(!Pause)){
        mem_addr = atoi(strtok(NULL, " "));

   //   if(mem_addr== 20007980){//version string address
        num_bytes = atoi(strtok(NULL, " "));
        i=0;
        UART_OutString(" ");
        while((i<num_bytes)&&(version_string[i]!=NULL)){
            UART_OutChar(version_string[i]);
            i++;
        }
        i=0;
        OutCRLF();
        UART_OutString("In hex format 0x");
        while((i<num_bytes)&&(version_string[i]!=NULL)){
            int ascii_value = (int)version_string[i];
            char ascii_char[4];
            sprintf(ascii_char, "%d", ascii_value);
            i++;
            UART_OutString(ascii_char);
            if((i!=num_bytes)&&(i<strlen(version_string)))//Dont print after last character
            UART_OutString("_");
     ...
     //  }
     //  else{
      //     UART_OutString(" Access not granted, please access register: 0x20007980");

      //}
    }

    else if((!(strcmp(cmd_string,"poke")))&&(!Stop)&&(!Pause))
    {
        mem_addr = atoi(strtok(NULL, " "));
     //  if(mem_addr==20007980){
        num_bytes = atoi(strtok(NULL, " "));
        cmd_string = strtok(NULL, " ");//string to overwrite existing data
        i=0;
        LCD_disp(1,0x01);//Clear display
        memcpy(version_string,cmd_string,num_bytes);
    ...
    //   }
     //  else{
      ...
      // }
        ...
        UART_OutString("System is paused, resume to use peek and poke functionalities");
    }
    else{
        UART_OutString("Given command is invalid.\n\rCommand Help\n\rValid commands are,\
                  ...
                  \n\r [2] blink <blink rate> ; blink rate must be an integer\n\r [3] start  \n\r [4] stop \
                   \n\r [5] pause  \n\r [6] resume  \n\r [7] peek memory_address_in_hex number_of_bytes_to_read; \
                   \n\r [8] poke memory_address_in_hex number_of_bytes_to_write content\n\r");
    }
    OutCRLF();
    Clear_InpBuff();
    UART_OutChar('>');
